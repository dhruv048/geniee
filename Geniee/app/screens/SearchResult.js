...
export default class SearchResult extends PureComponent {
 ...
 renderItem = (data, index) => {
    var item = data.item;
    return (
        <View key={item._id} style={{width:'33%', alignItems:'center', justifyContent:'center',marginTop:10}}>
        <TouchableOpacity style={styles.containerStyle} onPress={() => this._itemClick(item)}>
          <Body>
            <FAIcon name={item.icon} size={18} style={styles.catIcon} />
          </Body>
