...
export default class LocationPicker extends PureComponent {
    ...
    _handleLocationSelect(location){
      //  alert('clicked');
        ...
        fetch(`https://maps.googleapis.com/maps/api/place/details/json?place_id=${location.place_id}&fields=formatted_address,geometry,address_component,adr_address&key=${settings.GOOGLE_MAP_API_KEY}`)
            ...
            return (
                ...
                <View style={styles.container}>
                    ...
                    <Autocomplete
                        ...
                        data={this.state.locations.length === 1 && comp(query, this.state.locations[0].description) ? [] : this.state.locations}
                        defaultValue={query}
                        onChangeText={text =>{ this.setState({ query: text }), this.debounceFindLocation(text)}}
                        placeholder="Search Location"
                        renderItem={(item) => (
                            <ListItem key={item.id}   onPress={()=>this._handleLocationSelect(item)}>
                                <View>
                    ...
                    <View style={styles.deatilSection}>
                        ...
                        <Text numberOfLines={2} style={{ fontSize: 14, paddingVertical: 10, borderBottomColor: "silver", borderBottomWidth: 0.5 }}>
                            {!this.state.regionChangeProgress ? this.state.userLocation.formatted_address : "Identifying Location..."}</Text>
